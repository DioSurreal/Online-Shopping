...
type(
	PaymentUsecasesService interface{
		...
		BuyItem(pctx context.Context, cfg *config.Config, userId string, req *payment.ItemServiceReq) ([]*payment.PaymentTransferRes, error)
		SellItem(pctx context.Context, cfg *config.Config, userId string, req *payment.ItemServiceReq) ([]*payment.PaymentTransferRes, error)
	}
...
func (u *paymentUsecase) BuyItem(pctx context.Context, cfg *config.Config, userId string, req *payment.ItemServiceReq) ([]*payment.PaymentTransferRes, error) {
	...
	for _, item := range req.Items {
		u.paymentRepository.DockedUserMoney(pctx, cfg, &user.CreateUserTransactionReq{
			UserId: userId,
			Amount:   -item.Price,
		})
	...
	for _, s1 := range stage1 {
		if s1.Error != "" {
			for _, ss1 := range stage1 {
				u.paymentRepository.RollbackTransaction(pctx, cfg, &user.RollbackUserTransactionReq{
	...
	for _, s1 := range stage1 {
		u.paymentRepository.AddUserItem(pctx, cfg, &inventory.UpdateInventoryReq{
			UserId: userId,
			ItemId:   s1.ItemId,
		})
	...
	for _, s2 := range stage2 {
		if s2.Error != "" {
			for _, ss2 := range stage2 {
				u.paymentRepository.RollbackAddUserItem(pctx, cfg, &inventory.RollbackUserInventoryReq{
...
func (u *paymentUsecase) SellItem(pctx context.Context, cfg *config.Config, userId string, req *payment.ItemServiceReq) ([]*payment.PaymentTransferRes, error) {
	if err := u.FindItemsInIds(pctx, cfg.Grpc.ItemUrl, req.Items); err != nil {
		return nil, err
	}
	...
	stage1 := make([]*payment.PaymentTransferRes, 0)
	for _, item := range req.Items {
		u.paymentRepository.RemoveUserItem(pctx, cfg, &inventory.UpdateInventoryReq{
			UserId: userId,
			ItemId:   item.ItemId,
		})
		...
		if s1.Error != "" {
			for _, ss1 := range stage1 {
				if ss1.Error != "error: item not found" {
					u.paymentRepository.RollbackRemoveUserItem(pctx, cfg, &inventory.RollbackUserInventoryReq{
						UserId: userId,
						ItemId:   ss1.ItemId,
					})
				}
	...
	stage2 := make([]*payment.PaymentTransferRes, 0)
	for _, s1 := range stage1 {
		u.paymentRepository.AddUserMoney(pctx, cfg, &user.CreateUserTransactionReq{
			UserId: userId,
